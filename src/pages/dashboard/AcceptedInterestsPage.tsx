...
export const AcceptedInterestsPage = () => {
    ...
    const load = async (type: FilterType) => {
        try {
            setLoading(true);
            const data = await InterestsService.getAccepted(type);
            setInterests(data || []);
            setUserStatus(1); // Set state as healthy/active
        } catch (err: any) {
            console.error(err);
            setInterests([]);
    ...
    return (
        <DashboardLayout>
            <div className="space-y-6">
                {/* HEADER */}
                <div>
                    <h1 className="text-2xl font-bold text-foreground">Accepted Interests</h1>
                    {userStatus === 1 && (
                        <p className="text-sm text-muted-foreground mt-1">
                            Manage accepted interests
                        </p>
                    )}
                </div>
                                ...
                                <div className="relative z-10 space-y-4">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-muted-foreground text-sm max-w-sm mx-auto leading-relaxed">
                                            Connections that successfully progress past expressions of interest will be organized dynamically inside this logging timeline.
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="overflow-x-auto border rounded-xl bg-card shadow-sm">
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {loading ? (
                                            <tr>
                                                <td colSpan={5} className="p-8 text-center text-muted-foreground animate-pulse font-medium">
                                                    Loading accepted entries...
                                                </td>
                                            </tr>
                                        ) : (
                                            interests.map((interest) => (
                                                <tr key={interest.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                                                    ...
                                                    <td className="p-3">
                                                        <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100/80 shadow-none border-none font-semibold">
                                                            Accepted
                                                        </Badge>
                                                    </td>
                                                </tr>
...
export default AcceptedInterestsPage;