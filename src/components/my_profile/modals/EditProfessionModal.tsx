...
// Simplified props — no more states/cities/designations
interface Props {
  open: boolean;
  onClose: () => void;
  loading: boolean;
...
const EditProfessionModal = ({
  ...
  // Seed dropdown data when a record is opened for editing
  useEffect(() => {
    if (!open || !form?.profession_info_id) return;

    const load = async () => {
      if (form.profession_id) {
        const res = await masterService.getDesignationMaster(Number(form.profession_id));
        setDesignations(res || []);
      }
      if (form.country_id) {
        const res = await masterService.getStates(Number(form.country_id));
        setStates(res || []);
      }
      if (form.state_id) {
        const res = await masterService.getCities(Number(form.state_id));
        setCities(res || []);
      }
    };

    load();
  }, [open, form?.profession_info_id]);
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="max-w-4xl h-[90vh] p-0 overflow-hidden flex flex-col">
        ...
        <DialogFooter className="px-6 py-4 border-t shrink-0">
          <Button variant="outline" onClick={onClose} disabled={loading}>Discard</Button>
